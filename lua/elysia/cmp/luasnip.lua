...
local cfg = pRequire("elysia").config.cmp

local pathUtils = require("elysia.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "elysia", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "elysia", "cmp", "snippets", "vscode"),
})
